...
class ReviewControllerTest extends RestDocsSupport {
    ...
    void createReview() throws Exception {
        willDoNothing().given(reviewService).createReview(any(), anyLong(), any(ReviewRequest.class));
        ...
        verify(reviewService).createReview(any(), eq(1L), any(ReviewRequest.class));
    }

    @Test
    @DisplayName("리뷰 작성 실패 - 거래 완료 상태가 아닌 경우 400 반환")
    @WithMockCustomUser(memberId = 1L, email = "test@email.com", nickname = "테스트유저")
    void createReview_notSoldStatus_returns400() throws Exception {
        willThrow(new InvalidRequestException(ErrorCode.REVIEW_NOT_ALLOWED))
                .given(reviewService).createReview(any(), anyLong(), any(ReviewRequest.class));
    ...
    @Test
    @DisplayName("리뷰 작성 실패 - 구매자가 아닌 경우 403 반환")
    @WithMockCustomUser(memberId = 2L, email = "other@email.com", nickname = "다른유저")
    void createReview_notBuyer_returns403() throws Exception {
        willThrow(new ForbiddenException(ErrorCode.REVIEW_FORBIDDEN))
                .given(reviewService).createReview(any(), anyLong(), any(ReviewRequest.class));
    ...
    @Test
    @DisplayName("리뷰 작성 실패 - 이미 리뷰가 존재하는 경우 409 반환")
    @WithMockCustomUser(memberId = 1L, email = "test@email.com", nickname = "테스트유저")
    void createReview_alreadyExists_returns409() throws Exception {
        willThrow(new ConflictException(ErrorCode.REVIEW_ALREADY_EXISTS))
                .given(reviewService).createReview(any(), anyLong(), any(ReviewRequest.class));
    ...
    @Test
    @DisplayName("작성한 리뷰 목록 조회 - GET /members/{memberId}/reviews/written")
    @WithAnonymousUser
    void getWrittenReviews() throws Exception {
        CursorPageResponse<ReviewResponse> cursorResponse = new CursorPageResponse<>(
                reviewList(),
                "eyJpZCI6MX0=",
                true
        );
        given(reviewService.getWrittenReviews(anyLong(), any(), anyInt())).willReturn(cursorResponse);

        mockMvc.perform(get("/members/{memberId}/reviews/written", 1L)
                        .param("size", "10"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.code").value("SUCCESS"))
                .andDo(MockMvcRestDocumentationWrapper.document(
                        "review/get-written-reviews",
                        resource(ResourceSnippetParameters.builder()
                                .tag("Review")
                                .summary("작성한 리뷰 목록 조회")
                                .description("""
                                        특정 회원이 작성한 리뷰 목록을 커서 기반 페이지네이션으로 조회합니다.
                                        - 비로그인 상태에서도 조회 가능합니다.
                                        - 최신 리뷰가 먼저 반환됩니다.
                                ...
                                .responseFields(
                                        ...
                                        fieldWithPath("data.content[].reviewerId").type(JsonFieldType.NUMBER).description("리뷰 작성자 회원 ID"),
