...
> **버전**: v1.2 (UC41·UC42 추가 — 동네 인증·근처 상품 조회 / UC33 실제 구현 반영)
...
| 액터 | 비회원 |
| 사전 조건 | 가입된 이메일 |
| 기본 흐름 | 1. 이메일/비밀번호 입력 → 2. BCrypt 비교 → 3. AccessToken(15분) + RefreshToken(7일) 발급 → 4. RT Redis 저장 |
| 예외 흐름 | E1. 비밀번호 불일치 → 401 Unauthorized |
| 사후 조건 | AT(헤더) + RT(응답 바디) 반환 |
...
| 액터 | 비회원, 회원, 시스템(스케줄러) |
| 사전 조건 | 없음 |
| 기본 흐름 (클라이언트) | 1. GET /search/popular → 2. Caffeine 로컬 캐시에 `popularKeywords` 존재하면 반환 (만료시간 10분) → 3. 캐시 없으면 Redis `ZREVRANGE`로 상위 10개 실시간 연산 조회 → 4. 캐시에 저장 후 반환 (DB 미사용) |
| 기본 흐름 (스케줄러) | 캐시 만료 시 Redis에서 갱신하므로 DB 기반 스케줄러 미사용 (Redis 메모리 단독 집계) |
