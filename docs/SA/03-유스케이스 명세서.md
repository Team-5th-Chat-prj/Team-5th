...
> **버전**: v1.4 (코드 구현 기준 동기화 — RT 도입, Lettuce 분산락, Redis ZSET 인기검색어 반영)
...
| 기본 흐름 | 1. [예약하기] 클릭 → 2. Lettuce SETNX 분산락 획득 시도 (지수 백오프 스핀락) → 3. SELECT FOR UPDATE (비관적 락) → 4. status=SALE 확인 → 5. PRODUCT.status = RESERVED → 6. TRADE 생성 → 7. Lua 스크립트로 락 해제 → 8. 200 반환 |
| 예외 흐름 | E1. 락 획득 실패(재시도 초과) → 409 `ALREADY_RESERVED` / E2. 본인 상품 → 400 `SELF_RESERVATION` / E3. DB 락 타임아웃 → 409 `LOCK_TIMEOUT` |
...
|------|------|
| 액터 | 회원(구매자) |
| 사전 조건 | PRODUCT.status = TRADING AND TRADE.status = TRADING |
| 기본 흐름 | 1. 거래 완료 버튼 클릭 → 2. PATCH /trades/{id}/status { status: "SOLD_OUT" } → 3. TRADE.status = SOLD_OUT, PRODUCT.status = SOLD_OUT → 4. 200 반환 |
...
| 기본 흐름 | 1. 로그아웃 요청 → 2. Redis RT 삭제 → 3. Redis AT 블랙리스트 등록 (남은 만료시간 동안) → 4. 200 반환 |
| 예외 흐름 | E1. 이미 로그아웃 상태 → 401 Unauthorized (필터 레벨 차단) |
| 사후 조건 | 서버에서 토큰 무효화 처리. 재사용 시도 시 필터에서 차단됨. |
...
| 예외 흐름 | E1. SOLD_OUT 이후 취소 시도 → 400 / E2. 구매자가 TRADING 상태에서 취소 시도 → 403 `FORBIDDEN` |
| 사후 조건 | 상품 다시 SALE 상태로 복귀 |
...
| 기본 흐름 | 1. DELETE /chat-rooms/{chatRoomId}/leave → 2. 참여자 확인 → 3. 해당 참여자 플래그 Soft Delete (`deleted_by_buyer/seller = true`) → 4. 200 반환 |
| 설계 결정 | 양측이 모두 나갈 때까지 CHAT_ROOM 레코드는 유지되나, 나간 사용자에게는 목록에서 보이지 않음. 재입장 시(메시지 수신/전송) 복구 로직 포함. |
| 사후 조건 | 목록에서 제외됨 |
