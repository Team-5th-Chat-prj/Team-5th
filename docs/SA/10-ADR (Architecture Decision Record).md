...
## ADR-002. 비관적 락 + Lettuce 분산락 이중 방어 채택
...
| Lettuce | 확실 | DB 접근 전 차단 | 약간 상승 | 대응 가능 |
| Redisson | 확실 | DB 접근 전 차단 | 중간 (라이브러리 종속) | 대응 가능 |

### 결정

**비관적 락(필수) + Lettuce 기반 분산락(도전)** 순차 적용 (Redisson 대신 Spring Data Redis의 Lettuce 사용)

### 이유

1. 외부 라이브러리(Redisson) 의존도를 낮추기 위해 Spring 기본 패키지 생태계 내의 Lettuce 사용.
2. `StringRedisTemplate.setIfAbsent()` (SETNX) 및 Lua 스크립트를 통한 락 회수 학습 및 지식 축적 목적.
3. Redis 장애/응답 지연 시 비관적 락이 fallback으로 동작하는 이중 방어 구조 유지.
4. 사용자 정의 `LockService` 구현을 통해 Exponential Backoff 재시도 전략을 도입하여 DB와 Redis 양쪽 모두의 부하를 동시 경감 가능.
...
| Lettuce 락 | ✅ 동작 | ✅ 동작 — Redis 연결만 Cluster로 전환 |
| 문제 발생 시 | 없음 | DB가 다중화되면 비관적 락 효과 감소 → Lettuce 기반 분산락 필수화 |
