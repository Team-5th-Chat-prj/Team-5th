...
**JWT (AccessToken 15분 + RefreshToken 7일)** 채택 및 **서버측 무효화(블랙리스트)** 구현
...
RefreshToken 및 로그아웃 무효화 로직이 이미 구현 완료되었다.

| 항목 | 구현 내용 |
|------|------------|
| RefreshToken 발급 | JWT 생성 시 RT 발급, `RT:{memberId}` → `token` Redis 저장 (TTL 7일) |
| 토큰 재발급 API | `POST /auth/refresh` — RT Rotation (새 AT + 새 RT 발급) |
| 로그아웃 처리 | Redis AT 블랙리스트 등록 + RT 삭제 |
| 보안 강화 | RT 불일치 시 즉시 로그아웃 처리 (탈취 대응) |

> **관련 문서**: 04-기능명세서, 06-API명세서, 03-유스케이스(UC03·로그아웃) 동기화 완료
